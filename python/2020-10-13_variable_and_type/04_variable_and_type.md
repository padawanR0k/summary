...
순서가 존재하지 않으며 중복을 허용하지 않는 데이터 형태이다. 

세트 생성시에는 `{}` 를 사용한다. 이 때 중복으로 입력한 값은 하나만 남는다.
...
Set 타입은 교집합, 합집합, 차집합을 구하는 메소드를 제공한다. 연산자를 사용해도된다. 결과 또한 값의 중복을 허용하지 않는다.
...
이 세 타입은 서로 타입 변환이 가능하다.

튜플, 리스트 → 세트 타입변환은 중복처리가 되어 데이터가 변형되니 주의
...
key는 숫자나 문자열 타입으로 지정가능하며 value는 모든 데이터타입을 할당할 수 있다.
...
- 딕셔너리  데이터 다루기
    - js의 object와 비슷하게 `dict[key]`  키값을 사용해 값을 추가/수정한다
    - 특정 데이터를 삭제하기 위해서는  `del dict[key]` del 명령어를 사용한다.
    - 내장 메서드
        - `keys()`
        ...
        - `values()`
            ...
            딕셔너리 내부의 모든 value 리턴

        - `items()`

            딕셔너리 내부의 key, value를 튜플 형태로 변환하여 리턴

        - `update(dict_data2)`

            두 딕셔너리를 하나의 딕셔너리로 만든다. 이 때 메서드의 인수에 중복되는 값이 있다면 인수에 있는 값으로 덮어씌어진다.

        - `clear()`

            딕셔너리 내부에 모든 데이터를 지운다.

- 변수에 대해 수정 혹은 복사가 필요한 경우 유의! [딕셔너리는 mutable object이다.](https://velog.io/@muchogusto/call-by-reference-vs.-call-by-value)