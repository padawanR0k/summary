...
위 예제를 보면 변수 y에 문자열 ‘yyy’의 할당과 함수 bar가 실행된다.
...
전역 변수 x에 문자열'xxx'를 할당을 하면 , 현재 실행컨텍스트(global EC)의 스코프체인이 참조하고 있는 VO를 선두부터 검색하여 변수명에 해당하는 프로퍼티(x)가 발견되면 'xxx'를 할당한다.
...
함수foo가 실행되기 시작하면 새로운 함수 실행컨텍스트(foo() EC)가 생성된다.

foo() EC로 컨트롤이 이동하면 전역코드와 마찬가지로 
